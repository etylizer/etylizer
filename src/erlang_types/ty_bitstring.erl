...
% Native BDD substitution (bdd.hrl) calls ?ATOM:substitute/2 for dnf_ty_bitstring;
% a bitstring is a ty_tuple, so remap node refs via ty_tuple, like compare/equal.
-spec substitute(type(), #{ty_node:type() => ty_node:type()}) -> type().
substitute(P, NodeMap) -> ty_tuple:substitute(P, NodeMap).
