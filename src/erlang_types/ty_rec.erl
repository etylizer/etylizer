...
%%  io:format(user, "<~p> Prepared: ~n~p~n", [Ref, DnfMap]),
  ...
  Mapped = maps:map(fun({Pv, Nv}, TyR) ->
    ...
    case ty_rec:is_subtype(ty_rec:any(), TyR) of
      ...
      _ ->
        #ty{predef = P, atom = A, interval = I, list = L, map = M, tuple = {DT, T0, T1, Tn}, function = {DF, F}} = ty_ref:load(TyR),
...
prepare(TyRef) ->
  #ty{predef = P, atom = A, interval = I, list = L, tuple = {DT, T0, T1, Tn}, function = {DF, F}, map = M} = ty_ref:load(TyRef),
  VarMap = #{},
  ...
  AllKinds = lists:flatten([PMapped, AMapped, IMapped, LMapped, MMapped, TupleMapped, FunctionMapped, Tuple0Mapped, Tuple1Mapped, TupleExplicitMapped, FunctionExplicitMapped]),
...
extract_variables(Ty = #ty{ predef = P, atom = A, interval = I, list = L, tuple = Tall, function = F, map = M }) ->
...
negate(TyRef1) ->
  #ty{predef = P1, atom = A1, interval = I1, list = L1, tuple = {DT, T0, T1, Tn}, function = {DF, F}, map = M1} = ty_ref:load(TyRef1),
  ty_ref:store(#ty{
...
has_ref(#ty{map = Maps, list = Lists, tuple = {Default, T0, T1, AllTuple}, function = {DefaultF, AllFunction}}, TyRef) ->
  % TODO sanity remove
