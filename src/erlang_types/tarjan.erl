 ...
 roots(Graph, State, [V|Vs]) ->
   case maps:is_key(V, State#state.ixes) of
 ...
 from_root(Graph, State, V) ->
   ...
   NewState0 = State#state{
     ixes  = maps:put(V, -Me, State#state.ixes),
     lows  = maps:put(V, Me, State#state.lows),
     stack = [V | State#state.stack],
     num   = Me + 1
   },
   Adjacent = ?assert_type(element(?assert_type(V + 1, pos_integer()), Graph), [vertex()]), % FIXME dynamic call
   NewState1 = check_adj(Graph, NewState0, V, Adjacent),
   
   case maps:get(V, NewState1#state.lows) of
     X when X < Me -> NewState1;
     _ ->
       Stack = NewState1#state.stack,
       {Before, After} = lists:splitwith(fun(Vertex) -> Vertex =/= V end, Stack),
       case After of
         [B | Bs] ->
           This = [B | Before],
           N = NewState1#state.next_scc,
           Ixes1 = lists:foldl(fun(I, M) -> maps:put(I, N, M) end, 
                                 NewState1#state.ixes, This),
           NewState1#state{
             ixes     = Ixes1,
             stack    = Bs,
             sccs     = [{N, This} | NewState1#state.sccs],
             next_scc = N + 1
           };
         _ -> error(invariant)
       end
 ...
 map(Graph) ->
   ...
   AllNodes = [[Key | Values] || {Key, Values} <- maps:to_list(Graph)],
   All = lists:usort(lists:foldl(fun(E, Acc) -> Acc ++ E end, [], AllNodes)),
   ...
   Unfilled = maps:from_list(
     [{maps:get(Key, Mapping), [maps:get(Neighbor, Mapping) || Neighbor <- Neighbors]} 
      || {Key, Neighbors} <- maps:to_list(Graph)]
   ),
 ...
 condense(Graph) ->
   ...
   IdNodeMapping = maps:from_list([{Id, Node} || {Node, Id} <- maps:to_list(Mapping)]),
   ...
   NtoScc = maps:from_list([{maps:get(K, IdNodeMapping), V} || {K, V} <- maps:to_list(Map)]),
   
   % Build a mapping from each node to its root (SCC representative)
   %Roots = #{Node => hd(maps:get(Id, ScctoN)) || Node := Id <- NtoScc},
   Roots = maps:map(fun(_Node, Id) -> 
                            Ns = maps:get(Id, ScctoN),
                            hd(?assert_type(Ns, nonempty_list(vertex()))) % TODO document invariant
                            end, NtoScc), 
 ...
 -spec dfs_visit(Graph :: #{Node => [Node]}, Node, Acc :: [Node], VisitedSet :: sets:set(Node)) -> 
