...
-record(ctx,
        { path :: file:filename(),
          ...
          exhaustive_funs_toplevel = sets:new() :: sets:set({atom(), arity()})
        }).
-type ctx() :: #ctx{}.
    ...
    R =
        case Form of
            ...
            {attribute, _Anno, disable_exhaustiveness_toplevel, ListOfFuns} -> 
                {
                    ...
                    error,
                    Ctx#ctx { exhaustive_funs_toplevel = sets:union(Ctx#ctx.exhaustive_funs_toplevel, sets:from_list(ListOfFuns)) }
                };
            ...
            {function, Anno, Name, Arity, Clauses} ->
                case Mode of
                    full ->
                        DisableExhaustive = sets:is_element({Name, Arity}, Ctx#ctx.exhaustive_funs_toplevel),
