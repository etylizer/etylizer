...
-export([ variable/1, atom/1, interval/1, tuple/2]).
...
transform(TyRef, Ops =
  ...
  #ty{atom = A, interval = I, tuple = {DT, T}, function = {DF, F}} = Ty,
...
negate(TyRef1) ->
   #ty{atom = A1, interval = I1, tuple = {DT, T}, function = {DF, F}} = ty_ref:load(TyRef1),
  ty_ref:store(#ty{
