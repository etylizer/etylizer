...
exp_constrs(Ctx, E, T) ->
    case E of
        ...
        {lc, _L, _E, _Qs} -> halt(5);
        {map_create, _L, _Assocs} -> halt(5);
        {map_update, _L, _MapExp, _Assocs} -> halt(5);
        {nil, L} ->
        ...
        {'receive', _L, _CaseClauses} -> halt(5);
        {receive_after, _L, _CauseClauses, _TimeoutExp, _Body} -> halt(5);
        {record_create, _L, _Name, _Fields} -> halt(5);
        {record_field, _L, _Exp, _Name, _Field} -> halt(5);
        {record_index, _L, _Name, _Field} -> halt(5);
        {record_update, _L, _Exp, _Name, _Fields} -> halt(5);
        {tuple, L, Args} ->
            {Tys, Cs} =
                lists:foldr(
                  fun(Arg, {Tys, Cs}) ->
        ...
        {'try', _L, _Exps, _CaseClauses, _CatchClauses, _AfterBody} -> halt(5);
        {var, L, AnyRef} ->
