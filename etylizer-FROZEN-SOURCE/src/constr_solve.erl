...
% Check if a type is a bare tainted variable.
% Only matches bare variables, not variables inside intersections or other
% compound types, because intersections with concrete types (e.g. $0 /\ atom())
% indicate refinements that eliminate dynamism.
-spec has_tainted_var(ast:ty(), sets:set(ast:ty_varname())) -> boolean().
has_tainted_var({var, Alpha}, DynVars) -> sets:is_element(Alpha, DynVars);
has_tainted_var(_, _) -> false.

% Conditionally add a single element to a tainted set.
-spec taint_add_element(boolean(), ast:ty_varname(), sets:set(ast:ty_varname())) -> sets:set(ast:ty_varname()).
taint_add_element(true, Elem, Set) -> sets:add_element(Elem, Set);
taint_add_element(false, _, Set) -> Set.

% Conditionally add multiple variables to a tainted set.
-spec taint_add_vars(boolean(), [ast:ty_varname()], sets:set(ast:ty_varname())) -> sets:set(ast:ty_varname()).
taint_add_vars(true, Vars, Set) -> sets:union(Set, sets:from_list(Vars));
taint_add_vars(false, _, Set) -> Set.

% Compute the set of type variables transitively tainted by dynamic().
% A variable is tainted if dynamic() flows into it through any chain of
% scmater and scsubty constraints. Both source and target types of scsubty
% are traversed for nested variables (e.g. scsubty({$2,$1}, $0) adds edges
% from $2 and $1 to $0).
-spec dynamic_tainted_vars(
    [constr:simp_constr_subty() | constr:simp_constr_mater()],
    fun((ast:ty()) -> boolean())) -> sets:set(ast:ty_varname()).
dynamic_tainted_vars(AllConstrs, TyHasDynamic) ->
    {InitTainted, AdjMap} = lists:foldl(
        fun({scmater, _, Src, Alpha}, {Tainted, Adj}) ->
                T1 = taint_add_element(TyHasDynamic(Src), Alpha, Tainted),
                % Only propagate taint through bare variable materializations.
                % Intersections like $0 /\ atom() indicate guard/pattern refinements
                % that eliminate dynamism, so don't add edges through them.
                A1 = case Src of
                    {var, Beta} when is_atom(Beta) ->
                        maps:put(Beta, [Alpha | maps:get(Beta, Adj, [])], Adj);
                    _ -> Adj
                end,
                {T1, A1};
           ({scsubty, _, Src, Tgt}, {Tainted, Adj}) ->
                TgtVars = collect_vars(Tgt),
                T1 = taint_add_vars(TyHasDynamic(Src), TgtVars, Tainted),
                SrcVars = collect_vars(Src),
                A1 = add_var_edges_list(SrcVars, TgtVars, Adj),
                {T1, A1}
        end,
        {sets:new(), #{}},
        AllConstrs),
    propagate_taint(sets:to_list(InitTainted), InitTainted, AdjMap).

% Collect all type variable names occurring in a type.
-spec collect_vars(ast:ty()) -> [ast:ty_varname()].
collect_vars({var, Alpha}) when is_atom(Alpha) -> [Alpha];
collect_vars(Ty) ->
    utils:everything(
        fun ({var, Alpha}) when is_atom(Alpha) -> {ok, Alpha};
            (_) -> error
        end, Ty).

% Add flow edges from each source variable to each target variable.
-spec add_var_edges_list([ast:ty_varname()], [ast:ty_varname()],
    #{ast:ty_varname() => [ast:ty_varname()]}) -> #{ast:ty_varname() => [ast:ty_varname()]}.
add_var_edges_list(SrcVars, TgtVars, Adj) ->
    lists:foldl(
        fun(SrcVar, AccAdj) ->
            Old = maps:get(SrcVar, AccAdj, []),
            maps:put(SrcVar, TgtVars ++ Old, AccAdj)
        end,
        Adj,
        SrcVars).

% BFS propagation of taint through variable flow edges.
-spec propagate_taint([ast:ty_varname()], sets:set(ast:ty_varname()),
    #{ast:ty_varname() => [ast:ty_varname()]}) -> sets:set(ast:ty_varname()).
propagate_taint([], Tainted, _Adj) -> Tainted;
propagate_taint([Var | Rest], Tainted, Adj) ->
    Neighbors = maps:get(Var, Adj, []),
    {NewWork, NewTainted} = lists:foldl(
        fun(N, {W, T}) ->
            case sets:is_element(N, T) of
                true -> {W, T};
                false -> {[N | W], sets:add_element(N, T)}
            end
        end,
        {Rest, Tainted},
        Neighbors),
    propagate_taint(NewWork, NewTainted, Adj).


% Like has_dynamic_mater but takes pre-computed DynVars and TyHasDynamic.
-spec has_dynamic_mater_cached(constr:collected_constrs(), sets:set(ast:ty_varname()),
    fun((ast:ty()) -> boolean())) -> boolean().
has_dynamic_mater_cached(BranchConstrs, DynVars, TyHasDynamic) ->
    lists:any(
        fun ({scmater, _, Ty, _}) ->
                TyHasDynamic(Ty) orelse has_tainted_var(Ty, DynVars);
            (_) -> false
        end,
        sets:to_list(BranchConstrs)).

-spec check_redundant_branch_incr(symtab:t(), sets:set(ast:ty_varname()),
    tally:base_sat_result(), sets:set(ast:ty_varname()), fun((ast:ty()) -> boolean()),
    {ast:loc(), constr:collected_constrs()}, ok | {error, error()}) -> ok | {error, error()}.
check_redundant_branch_incr(_Tab, _FixedTyvars, _BaseResult, _DynVars, _TyHasDynamic, _LocAndConstrs, Acc = {error, _}) -> Acc;
check_redundant_branch_incr(Tab, FixedTyvars, BaseResult, DynVars, TyHasDynamic, {Loc, UnmatchedConstrs}, ok) ->
    case has_dynamic_mater_cached(UnmatchedConstrs, DynVars, TyHasDynamic) of
        true ->
            ?LOG_DEBUG("Branch at ~s: skipping redundancy check (dynamic materialization)",
                ast:format_loc(Loc)),
            ok;
        false ->
                    ...
                    case tally:is_satisfiable_base(Tab, SubtyConstrs, FixedTyvars) of
                        {true, BaseResult} ->
                            ?LOG_DEBUG("Constraints are satisfiable, now checking ~w branches for redundancy (incremental)",
                                length(ReduDs)),
                            % Precompute dynamic taint analysis once for all branches.
                            TyHasDynamic = fun(Ty) ->
                            ...
                            DynVars = dynamic_tainted_vars(sets:to_list(SubtyConstrs), TyHasDynamic),
                            lists:foldl(
                                fun (LocAndConstrs, Acc) ->
                                    check_redundant_branch_incr(Tab, FixedTyvars, BaseResult, DynVars, TyHasDynamic, LocAndConstrs, Acc)
                                end,
                                ok,
                                ReduDs);
                        {false, _} ->
