...
erlang_atom() -> % a restricted version of the Erlang atom
    choice([
        around(string("'"), bind(word(), fun(Str) -> result(list_to_existing_atom(Str)) end), string("'")),
        bind(lower(), fun(Start) -> bind(word(), fun(Rest) -> result(list_to_existing_atom([Start] ++ Rest)) end) end)
    ]).
