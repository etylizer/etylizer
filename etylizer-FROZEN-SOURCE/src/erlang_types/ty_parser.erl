...
do_convert_rewrite({{var, A}, R}, Q, Cache) ->
  ...
  case string:prefix(atom_to_list(A), "$ety_") of
    ...
    IdName ->
      ...
      {?TY:singleton(ty_variable:with_name_and_id(list_to_integer(Id), list_to_existing_atom(Name))), Q, R, Cache}
  end;
...
do_convert_data({{improper_list, A, B}, R}, Q, Cache) ->
  RVar = {mu_var, list_to_existing_atom(integer_to_list(erlang:unique_integer()))},
...
debruijn({mu_var, Name}, Env) ->
  case index_of(Name, Env) of
    {ok, Index} -> {mu_var, list_to_existing_atom(integer_to_list(Index))};
...
make_fresh_name(Counter) ->
  list_to_existing_atom("$var_" ++ integer_to_list(Counter)).
