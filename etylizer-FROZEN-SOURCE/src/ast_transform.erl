...
get_fresh_var_name(Ctx) ->
    ...
    FreshVar = list_to_existing_atom("$maybe" ++ integer_to_list(Counter)),
...
test_ctx() ->
    #ctx{ path = ".", module_name = ast_transform, funenv = varenv:empty("function"), records = #{}, fresh_var_counter = 1 }.
...
rewrite_dispatched_clauses(Clauses, PositionTypes) ->
    ...
    FreshVars = [begin
        Name = list_to_existing_atom("$dispatch_" ++ integer_to_list(I)),
...
store_record_variant(RecName, Overrides) ->
    ...
    N = length(Existing) + 1,
    VariantName = list_to_existing_atom("$record$" ++ atom_to_list(RecName) ++ "$v" ++ integer_to_list(N)),
