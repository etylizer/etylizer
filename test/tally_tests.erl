...
test_tally_unique(ConstrList, ExpectedSubst, FixedVars) ->
    ...
    case tally:tally(symtab:empty(), Constrs, sets:from_list(FixedVars)) of
        [RealSubst] ->
            ...
            lists:foreach(fun({Var, Ty}) ->
              ?assertEqual(true,
                begin
                  TyOther = maps:get(Var, RealSubst),
                  subty:is_equivalent(none, Ty, TyOther)
                end
              )
                          end, maps:to_list(ExpectedSubst));
...
%%tally_01_test() ->
%%    test_tally_unique([{tvar(alpha), tint()}], #{ alpha => tintersect([tvar(mu2), tint()])}).
%%
%%tally_02_test() ->
%%    test_tally_unique([{tint(), tvar(alpha)}], #{ alpha => tunion([tvar(mu2), tint()]) }).
