...
-spec intersect_combiner_v1(Key, Value1, Value2) -> Value1 when Key :: term(), Value1 :: term(), Value2 :: term().
-spec intersect_combiner_v2(Key, Value1, Value2) -> Value2 when Key :: term(), Value1 :: term(), Value2 :: term().
-spec intersect_with_small_map_first(Combiner, SmallMap, BigMap) -> Map when Combiner :: fun((Key, Value1, Value2) -> CombineResult), SmallMap :: #{Key => Value1}, BigMap :: #{Key => Value2}, Map :: #{Key => CombineResult}.
-spec intersect_with_iterate(NextResult, Keep, BigMap, Combiner) -> Map when NextResult :: {Key, Value1, iterator(Key, Value1)} | none, Keep :: [{Key, CombineResult}], BigMap :: #{Key => Value2}, Combiner :: fun((Key, Value1, Value2) -> CombineResult), Map :: #{Key => CombineResult}.
-spec merge_with_1(NextResult, Map1, Combiner) -> Map2 when NextResult :: {Key, Value2, iterator(Key, Value2)} | none, Map1 :: #{Key => Value1}, Combiner :: fun((Key, Value1, Value2) -> CombineResult), Map2 :: #{Key => CombineResult}.
-spec to_list_from_iterator(NextResult) -> [{Key, Value}] when NextResult :: {Key, Value, iterator(Key, Value)} | none.
-spec to_list_internal(Acc) -> [{Key, Value}] when Acc :: ([{Key, Value}] | [{integer(), map()}]).
-spec filter_1(Pred, NextResult, ErrorTag) -> [{Key, Value}] when Pred :: fun((Key, Value) -> boolean()), NextResult :: {Key, Value, iterator(Key, Value)} | none, ErrorTag :: reference().
-spec foreach_1(Fun, NextResult, ErrorTag) -> ok when Fun :: fun((Key, Value) -> term()), NextResult :: {Key, Value, iterator(Key, Value)} | none, ErrorTag :: reference().
-spec map_1(Fun, NextResult, ErrorTag) -> [{Key, Value2}] when Fun :: fun((Key, Value1) -> Value2), NextResult :: {Key, Value1, iterator(Key, Value1)} | none, ErrorTag :: reference().
-spec with_1(Ks, Map) -> [{K, V}] when Ks :: [K], Map :: #{K => V}.
-spec groups_from_list_1(KeyFun, List, Acc) -> GroupsMap when KeyFun :: fun((Elem) -> Key), List :: [Elem], Acc :: #{Key => Group}, GroupsMap :: #{Key => Group}.
-spec error_type(M) -> badarg | {badmap, term()} when M :: term().
-spec error_type_two_maps(M1, M2) -> {badmap, term()} when M1 :: term(), M2 :: term().
-spec error_type_merge_intersect(M1, M2, Combiner) -> badarg | {badmap, term()} when M1 :: term(), M2 :: term(), Combiner :: term().
-spec badarg_with_info(Args) -> no_return() when Args :: list().
-spec error_with_info(Reason, Args) -> no_return() when Reason :: term(), Args :: list().
-spec is_iterator_valid_1(Iter) -> boolean() when Iter :: iterator() | {term(), term(), iterator()} | none.
-spec try_next(Iter, ErrorTag) -> {Key, Value, iterator(Key, Value)} | none when Iter :: iterator(Key, Value), ErrorTag :: reference() | undefined.

% TODO errors
-spec groups_from_list_2(KeyFun, ValueFun, List, Acc) -> GroupsMap when KeyFun :: fun((Elem) -> Key), ValueFun :: fun((Elem) -> _Value), List :: [Elem], Acc :: #{Key => Group}, GroupsMap :: #{Key => Group}.
-spec filtermap_1(Fun, NextResult, ErrorTag) -> [{Key, Value}] when Fun :: fun((Key, Value1) -> boolean() | {true, Value}), NextResult :: {Key, Value1, iterator(Key, Value)} | none, ErrorTag :: reference().
-spec fold_1(Fun, Acc, NextResult, ErrorTag) -> Acc when Fun :: fun((Key, Value, _AccIn) -> _AccOut), Acc :: term(), NextResult :: {Key, Value, iterator(Key, Value)} | none, ErrorTag :: reference().
