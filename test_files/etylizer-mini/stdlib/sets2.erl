...
-spec intersection_heuristic(
        maps:iterator(Element, ?VALUE), 
        [Element],
        [Element], integer(), 
        #{Element => []}, 
        #{Element => []}) -> #{Element => []}.
...
intersection_heuristic(_, _, _, _, _, _) ->
...
intersection_decided(none, Acc, _Reference) ->
    Acc;
intersection_decided(_, _Acc, _Reference) -> error(exhaustiveness).
...
-spec subtract_heuristic(maps:iterator(Element, ?VALUE), [Element], [Element], non_neg_integer(), 
...
subtract_heuristic(none, Keep, _Delete, _Count, _Acc, _Reference) ->
    maps:from_keys(Keep, ?VALUE);
subtract_heuristic(_, _Keep, _Delete, _Count, _Acc, _Reference) ->
  error(exhaustiveness).
...
-spec filter_seg_list(
        fun((_Element) -> boolean()), 
          [seg()], 
          [seg()], 
          non_neg_integer()) -> {[seg()], non_neg_integer()}.
