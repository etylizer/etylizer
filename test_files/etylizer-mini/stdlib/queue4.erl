...
% -spec get(list(), list()) -> term(). CHANGE
get(_R, [H|_]) ->
    H;
get([H], []) -> H; % scrut: list(Item) \ {cons, H} 
get([_|R], []) ->  % R cant be []
  % Alt
  % lists:last(R);
  ...
  % Neu
  case R of
    [] -> error(list_refinement);
    Z ->lists:last(Z)
  end;


get(_, _) -> error(todo_exhaustive). %TODO explain invariant cant be captured just with the function 
...
peek({[_|R],[]}) -> % malformed queue -> O(len(Q))
    case R of [] -> error(refinement); _ -> {value,lists:last(R)} end.
% peek(Q) ->
%     erlang:error(badarg, [Q]).
...
-spec test(integer(), [Item], [Item], [Item], [Item]) -> {queue(Item), queue(Item)}.
test(0, R1, F1, R2, F2) ->
    {{R2,F2},{R1,F1}};
test(N, R1, [X|F1], R2, F2) ->
    test(N-1, R1, F1, [X|R2], F2);
test(N, R1, _, R2, F2) ->
  error(exhaustive).
% test(_, _, _, _, _) -> error(exhaustive).
...
split_f1_to_r2(_, _, _, _, _) -> error(exhaustive).
