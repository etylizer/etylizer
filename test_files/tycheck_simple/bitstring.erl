...
-spec b5_embed_bitstring() -> binary().
b5_embed_bitstring() ->
  <<"abc">> = <<(<<"abc">>)/bitstring>>,
  <<"abc">>.

-spec b5_embed_binary_unit1() -> binary().
b5_embed_binary_unit1() ->
  <<"abc">> = <<(<<"abc">>)/binary-unit:1>>,
  <<"abc">>.

-spec b5_embed_binary() -> binary().
b5_embed_binary() ->
  <<"abc">> = <<(<<"abc">>)/binary>>,
  <<"abc">>.

-spec b5_embed_1bit_bitstring() -> bitstring().
b5_embed_1bit_bitstring() ->
  <<1:1>> = <<(<<1:1>>)/bitstring>>,
  <<1:1>>.

-spec b5_embed_1bit_binary_unit1() -> bitstring().
b5_embed_1bit_binary_unit1() ->
  <<1:1>> = <<(<<1:1>>)/binary-unit:1>>,
  <<1:1>>.

-spec b5_match_binary_unit16_empty() -> binary().
b5_match_binary_unit16_empty() ->
  <<_/binary-unit:16>> = <<"">>,
  <<>>.

-spec b5_match_binary_unit16_2bytes() -> binary().
b5_match_binary_unit16_2bytes() ->
  <<_/binary-unit:16>> = <<"ab">>,
  <<"ab">>.

-spec b5_match_binary_unit16_4bytes() -> binary().
b5_match_binary_unit16_4bytes() ->
  <<_/binary-unit:16>> = <<"abcd">>,
  <<"abcd">>.

-spec b5_truncate_binary() -> binary().
b5_truncate_binary() ->
  <<"ab">> = <<(<<"abc">>):2/binary>>,
  <<"ab">>.
...
bin_match_int_01(<<A:4, B:4, _/binary>>) -> A + B;
bin_match_int_01(<<>>) -> error(exhausitve).

-spec bin_match_int_01_fail(binary()) -> integer().
bin_match_int_01_fail(<<A:4, B:4, _/binary>>) -> A + B.


-spec bin_match_int_02_fail(binary()) -> integer().
bin_match_int_02_fail(<<A, _/binary>>) -> A.

-spec bin_match_int_02(binary()) -> integer().
bin_match_int_02(<<A, _/binary>>) -> A;
bin_match_int_02(<<>>) -> error(exhaustive).

-spec bin_match_int_03(binary()) -> integer().
bin_match_int_03(<<A, B, _/binary>>) -> A + B;
bin_match_int_03(<<_>>) -> error(exhaustive);
bin_match_int_03(<<>>) -> error(exhaustive).

-spec bin_match_int_03_fail(binary()) -> integer().
bin_match_int_03_fail(<<A, B, _/binary>>) -> A + B.

% Match with explicit integer type
-spec bin_match_int_04(binary()) -> integer().
bin_match_int_04(<<A:16/integer, _/binary>>) -> A;
bin_match_int_04(<<_>>) -> error(exhaustive);
bin_match_int_04(<<>>) -> error(exhaustive).

-spec bin_match_int_04_fail(binary()) -> integer().
bin_match_int_04_fail(<<A:16/integer, _/binary>>) -> A.

% Match with little-endian
-spec bin_match_int_05(binary()) -> integer().
bin_match_int_05(<<A:16/little, _/binary>>) -> A;
bin_match_int_05(<<_>>) -> error(ex);
bin_match_int_05(<<>>) -> error(ex).

-spec bin_match_int_05_fail(binary()) -> integer().
bin_match_int_05_fail(<<A:16/little, _/binary>>) -> A.

% Match signed integer
-spec bin_match_int_06(binary()) -> integer().
bin_match_int_06(<<A:8/signed, _/binary>>) -> A;
bin_match_int_06(<<>>) -> error(ex).

-spec bin_match_int_06_fail(binary()) -> integer().
bin_match_int_06_fail(<<A:8/signed, _/binary>>) -> A.

% Match unsigned integer (default)
-spec bin_match_int_07(binary()) -> non_neg_integer().
bin_match_int_07(<<A:8/unsigned, _/binary>>) -> A;
bin_match_int_07(<<>>) -> error(ex).

-spec bin_match_int_07_fail(binary()) -> non_neg_integer().
bin_match_int_07_fail(<<A:8/unsigned, _/binary>>) -> A.
...
-spec bin_match_float_01_fail(binary()) -> float().
bin_match_float_01_fail(<<F:64/float, _/binary>>) -> F.

-spec bin_match_float_01(binary()) -> float().
bin_match_float_01(<<F:64/float, _/binary>>) -> F;
bin_match_float_01(<<_:56, _/binary>>) -> error(exhaustive); 
bin_match_float_01(<<_:48, _/binary>>) -> error(exhaustive); 
bin_match_float_01(<<_:40, _/binary>>) -> error(exhaustive); 
bin_match_float_01(<<_:32, _/binary>>) -> error(exhaustive); 
bin_match_float_01(<<_:24, _/binary>>) -> error(exhaustive); 
bin_match_float_01(<<_, _/binary>>) -> error(exhaustive); 
bin_match_float_01(<<>>) -> error(exhaustive).

% Match a 32-bit float
-spec bin_match_float_02(binary()) -> float().
bin_match_float_02(<<F:32/float, _/binary>>) -> F;
bin_match_float_02(<<_:24, _/binary>>) -> error(exhaustive); 
bin_match_float_02(<<_, _/binary>>) -> error(exhaustive); 
bin_match_float_02(<<>>) -> error(exhaustive).
...
% with etylizer they fail: not exhaustive
% ============================================================

% Default (unsigned) integer from binary
-spec bin_signed_01_fail(binary()) -> non_neg_integer().
bin_signed_01_fail(B) ->
    <<A>> = B,
    A.

% Explicit unsigned integer
-spec bin_signed_02_fail(binary()) -> non_neg_integer().
bin_signed_02_fail(B) ->
    <<A/unsigned>> = B,
    A.

% Signed integer
-spec bin_signed_03_fail(binary()) -> integer().
bin_signed_03_fail(B) ->
    <<A/signed>> = B,
    A.
...
-spec bin_let_01_fail(binary()) -> {integer(), integer()}.
bin_let_01_fail(Bin) ->
    <<A, B, _/binary>> = Bin,
    {A, B}.
...
bin_let_01(Bin) ->
    <<A, B, _/binary>> = ?assert_pattern(<<_, _, _/binary>>, Bin),
    {A, B}.

% Nested binary match
-spec bin_let_02(binary()) -> {integer(), binary()}.
bin_let_02(Bin) ->
    <<Len, Rest/binary>> = ?assert_pattern(<<_, _/binary>>, Bin),
    <<Payload:Len/binary, _/binary>> = ?assert_pattern(<<_, _/binary>>, Rest),
    {Len, Payload}.
...
% TODO: integer segment variables get non_neg_integer() instead of the precise
% value from the scrutiny. To fix, the solver would need to project singleton
% bits back into integer values.
-spec bin_let_04() -> {1, 2, 3}.
bin_let_04() ->
    Bin = <<1, 2, 3>>,
    <<A, Rest1/binary>> = Bin,
    <<B, Rest2/binary>> = Rest1,
...
% ============================================================
% === Binary cons: content-based pattern discrimination
% ============================================================

% Different leading byte values should not be redundant
-spec bin_cons_01(binary()) -> a | b | other.
bin_cons_01(<<1, _/binary>>) -> a;
bin_cons_01(<<2, _/binary>>) -> b;
bin_cons_01(_) -> other.

% Same leading byte: second clause is redundant
-spec bin_cons_02_fail(binary()) -> a | b | other.
bin_cons_02_fail(<<1, _/binary>>) -> a;
bin_cons_02_fail(<<1, _/binary>>) -> b;
bin_cons_02_fail(_) -> other.

% Different two-byte prefixes (like the number_exponation_part example)
-spec bin_cons_03(binary()) -> a | b | c | other.
bin_cons_03(<<$e, $+, _/binary>>) -> a;
bin_cons_03(<<$e, $-, _/binary>>) -> b;
bin_cons_03(<<$E, _/binary>>) -> c;
bin_cons_03(_) -> other.

% Wildcard byte followed by specific byte: not redundant
-spec bin_cons_04(binary()) -> a | b | other.
bin_cons_04(<<_, 1, _/binary>>) -> a;
bin_cons_04(<<_, 2, _/binary>>) -> b;
bin_cons_04(_) -> other.

% First clause covers all leading bytes, second is redundant
-spec bin_cons_05_fail(binary()) -> a | b | other.
bin_cons_05_fail(<<_, _/binary>>) -> a;
bin_cons_05_fail(<<1, _/binary>>) -> b;
bin_cons_05_fail(_) -> other.


% ============================================================
% === UTF-8 pattern discrimination
% ============================================================

% Different literal UTF-8 values should not be redundant
-spec bin_utf8_cons_01(binary()) -> a | b | other.
bin_utf8_cons_01(<<$a/utf8, _/binary>>) -> a;
bin_utf8_cons_01(<<$b/utf8, _/binary>>) -> b;
bin_utf8_cons_01(_) -> other.

% Same literal UTF-8 value: second clause is redundant
-spec bin_utf8_cons_02_fail(binary()) -> a | b | other.
bin_utf8_cons_02_fail(<<$a/utf8, _/binary>>) -> a;
bin_utf8_cons_02_fail(<<$a/utf8, _/binary>>) -> b;
bin_utf8_cons_02_fail(_) -> other.

% 2-byte UTF-8: different codepoints (U+00E9 vs U+00F1, both 2-byte)
-spec bin_utf8_cons_03(binary()) -> a | b | other.
bin_utf8_cons_03(<<16#E9/utf8, _/binary>>) -> a;   % é
bin_utf8_cons_03(<<16#F1/utf8, _/binary>>) -> b;   % ñ
bin_utf8_cons_03(_) -> other.

% 3-byte UTF-8: different codepoints (U+4E16 vs U+754C, both 3-byte)
-spec bin_utf8_cons_04(binary()) -> a | b | other.
bin_utf8_cons_04(<<16#4E16/utf8, _/binary>>) -> a;  % 世
bin_utf8_cons_04(<<16#754C/utf8, _/binary>>) -> b;  % 界
bin_utf8_cons_04(_) -> other.

% 4-byte UTF-8: different codepoints (U+1F600 vs U+1F60A, both 4-byte)
-spec bin_utf8_cons_05(binary()) -> a | b | other.
bin_utf8_cons_05(<<16#1F600/utf8, _/binary>>) -> a;  % 😀
bin_utf8_cons_05(<<16#1F60A/utf8, _/binary>>) -> b;  % 😊
bin_utf8_cons_05(_) -> other.

% Mixed UTF-8 sizes: 1-byte vs 2-byte vs 3-byte should not be redundant
-spec bin_utf8_cons_06(binary()) -> a | b | c | other.
bin_utf8_cons_06(<<$a/utf8, _/binary>>) -> a;        % 1-byte
bin_utf8_cons_06(<<16#E9/utf8, _/binary>>) -> b;     % 2-byte
bin_utf8_cons_06(<<16#4E16/utf8, _/binary>>) -> c;   % 3-byte
bin_utf8_cons_06(_) -> other.
